# TimeSliver - EEG Sleep Stage Classification

An interpretable time series classification pipeline for EEG-based sleep stage classification using CNN-based motif extraction with temporal attribution capabilities.
...
│   ├── y_train.npy             # Training labels (5 sleep stages)
...
1. Loads raw EEG sleep stage data
2. Splits data: train, valid, test (stratified)
3. Applies SAX tokenization (alphabet_size=25, word_length=1)
...
|----------|-----------|---------|
| Data | `SAX_ALPHABET_SIZE` | 25 |
| Data | `NUM_CLASSES` | 5 (sleep stages) |
| Data | `INPUT_DIM` | 1 (single-channel EEG) |
| Model | `D_MODEL` | 32 |
| Model | `D_OUT` | 12 |
| Model | `MAX_M` | 1 |
| Training | `TRAIN_BATCH_SIZE` | 512 |
| Training | `TRAIN_LR` | 0.001 |
| Training | `TRAIN_EPOCHS` | 2500 |
...
### `main/timesliver.py`
Core network architecture for EEG sleep stage classification.
...
- **Architecture:**
  ...
  Input [batch, seq_len, 1]
      ↓
  Linear Projection (1 → 32) → Positional Encoding
      ↓
  CNN Stack (Scale 1): Conv1d(32→16→32→12, kernels=[2,5,5])
      ↓
  SAX Pooling: AvgPool1d (3 layers, kernels=[2,5,5])
      ↓
  Motif-Document Interaction Matrix
      ↓
  AvgPool2d Reduction → Linear(d_out*12, 5)
  ```

- **Multi-scale Motif Support (cnn1, cnn2, cnn3):**
  - Scale 1: 10-size motifs
  - Scale 2: 100-size motifs
  - Scale 3: 400-size motifs
...
python train_model.py --batch-size 256 --d-model 64
...
| `--d-model` | 32 | Model dimension (q) |
| `--d-out` | 12 | CNN output dim (d) |
...
**Output:** Accuracy and 5x5 confusion matrix
...
4. `assigning_importance()`: Map reduced-length importance back to original positions
5. Apply edge weighting for convolution boundary effects

**Outputs:** `model/importance_{train,valid,test}.npy` (shape: samples × seq_len)
...
│                         DATA PREPARATION                            │
│  run_data_prep.ipynb                                                │
│  Raw EEG → SAX Encoding → One-Hot → Train/Valid/Test Split          │
└─────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────┐
│                         BASELINE TRAINING                           │
│  python train_model.py --epochs 2500 --lr 0.001                     │
│  → model/best.pth                                                   │
└─────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────┐
...
│  → Accuracy, 5x5 Confusion Matrix (masked)                          │
└─────────────────────────────────────────────────────────────────────┘
```
...
cd main
python train_model.py --epochs 2500 --device cuda
...
## Key Differences from FordA

| Aspect | EEG Sleep Stage | FordA |
|--------|-----------------|-------|
| Input dimension | 1 (single-channel) | 70 (one-hot SAX) |
| Number of classes | 5 (sleep stages) | 2 (binary) |
| SAX alphabet size | 25 | 70 |
| Model dim (d_model) | 32 | 70 (from data) |
| CNN output (d_out) | 12 | 36 |
| Motif sizes | 10, 100, 400 | 10, 6 |
| Pooling kernels | [2,5,5] | [4,4,4] |
| Final layer input | d_out * 12 | 18 * 35 |
...
6. **Multi-scale Motifs**: Architecture supports 3 motif scales (10, 100, 400) though only scale 1 is used by default
