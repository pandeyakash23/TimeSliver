"""
TimeSliver Network Architecture for EEG Sleep Stage Classification.
...
class TimeSliverNetwork(nn.Module):
    """
    TimeSliver Network for EEG Sleep Stage Classification.

    Combines CNN-based motif extraction with SAX pooling for
    interpretable time series classification. Uses single-channel
    EEG input with 5-class sleep stage output.
    """
    ...
    def __init__(self, num_classes, d_model, d_out, max_m, device):
        ...
        Args:
            num_classes: Number of output classes (5 for sleep stages)
            d_model: Model dimension (default: 32)
            d_out: Output dimension for CNN (default: 12)
        ...
        self.max_m = max_m
        self.device = device
        self.d_in = 1  # Single-channel EEG
        self.d_model = d_model
        self.d_out = d_out
        self.sax_alpha = 25  # SAX alphabet size for EEG

        # Projection layer (from 1D input to d_model)
        self.proj = nn.Linear(1, self.d_model)

        # Positional encoding
        self.positional_encoding = PositionalEncoding(self.d_model)

        # CNN layers for motif extraction - Scale 1 (10-size motifs)
        self.cnn1 = nn.Sequential(
            nn.Conv1d(self.d_model, 16, 2, stride=1),
            nn.ReLU(),
            nn.Conv1d(16, 32, 5, stride=1),
            nn.ReLU(),
            nn.Conv1d(32, int(self.d_out), 5, stride=1),
        )
        self.ln1 = nn.LayerNorm(self.d_out)

        # CNN layers for motif extraction - Scale 2 (16-size motifs)
        self.cnn2 = nn.Sequential(
            ...
            nn.Conv1d(32, int(self.d_out), 5, stride=1),
            nn.ReLU(),
            nn.Conv1d(int(self.d_out), int(self.d_out), 5, stride=1),
        )
        self.ln2 = nn.LayerNorm(self.d_out)

        # CNN layers for motif extraction - Scale 3 (16-size motifs)
        ...
        # Layer norms
        self.start = nn.LayerNorm(self.d_model)
        self.ln4 = nn.LayerNorm(self.sax_alpha)

        # SAX pooling layers - Scale 1
        self.maxpool1 = nn.Sequential(
            nn.AvgPool1d(2, stride=1),
            nn.AvgPool1d(5, stride=1),
            nn.AvgPool1d(5, stride=1),
        )

        # SAX pooling layers - Scale 2
        ...
        # Motif sizes for different scales
        self.motif_size_1 = 10
        self.motif_size_2 = 100
        self.motif_size_3 = 400

        # Reduction and classification layers
        self.reduction = nn.Sequential(nn.AvgPool2d((1, 2), (1, 2)))
        self.nn = nn.Sequential(nn.Linear(int(self.d_out * 12 * self.max_m), num_classes))
    ...
    def forward(self, x, sax, seq_len):
        ...
        # Get motif-document interaction (using scale 1 only by default)
