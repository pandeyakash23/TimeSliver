...
class timesliver_network(nn.Module):
    def __init__(self, num_classes, d_model, d_out, max_m, rank):
        ...
        self.cnn2 = nn.Sequential( nn.Conv1d(self.d_model,16,2, stride=1), 
                                   nn.ReLU(),
        ...
        self.cnn3 = nn.Sequential( nn.Conv1d(self.d_model,16,4, stride=1), 
                                   nn.ReLU(),
        ...
        self.maxpool3 = nn.Sequential( nn.AvgPool1d(4, stride=1),
                                       nn.AvgPool1d(4, stride=1),
                                       nn.AvgPool1d(5, stride=1),
                                       nn.AvgPool1d(5, stride=1),
                                   )
        
        self.motif_size_1 = 10*1
        self.motif_size_2 = 100*1
        self.motif_size_3 = 400*1
        
        self.reduction = nn.Sequential(nn.AvgPool2d((1,2),(1,2)))
                      
        self.nn = nn.Sequential(
                                nn.Linear(int(self.d_out*12*self.max_m),num_classes),
                                # nn.Linear(int(sax_alpha*self.d_out*self.max_m),32),
                                # nn.ReLU(),
                                # nn.Linear(32,8),
                                # nn.ReLU(),
    ...
    def forward(self, x, sax, seq_len):
        ...
        # _,_,out_3 = self.make_p(out, sax,seq_len,3)
        
        # heat_map = torch.cat((out_1, out_2, out_3), dim=3)      
        heat_map = out_1
    ...
    def forward_motif_importance(self, x,  sax, seq_len):
        ...
        # self.out_3, self.pool_3,p_3 = self.make_p(out, sax,seq_len,3)
        
        # heat_map = torch.cat((p_1, p_2,p_3), dim=3)    
        heat_map = p_1
