"""
Calculate temporal attribution scores for audio samples.

This script computes per-time-point importance scores that identify
which parts of the audio sequence are most relevant for the model's predictions.
...
import config
import utils
from timesliver import TimeSliverNetwork, TimeSlicerDataset


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Calculate temporal attribution scores for audio data",
...
def calculate_attribution(model, dataloader, device, num_samples, max_seq_len):
    ...
    for batch_idx, (x, sax, classes, seq_len, labels) in enumerate(dataloader):
        ...
        print(f"  Batch {batch_idx+1}: cam size {cam.size()}, initial_cam size {initial_cam.size()}")

        # Normalize gradients per sample
        for prot in range(cam.size(0)):
            cam[prot, ...] = cam[prot, ...] / (abs(torch.max(cam[prot, ...])) + 1e-18)
            initial_cam[prot, ...] = initial_cam[prot, ...] / (
                abs(torch.max(initial_cam[prot, ...])) + 1e-18
            )

        # Calculate motif-level importance
        for m_i in range(cam.size(-1)):
            mo_level_imp = model.calculate_motif_level(cam[..., m_i], m_i + 1, initial_cam)
            ...
            head_kernel = kernel_size / torch.arange(1, kernel_size).to(device)
            tail_kernel = head_kernel.flip(0)

            store_importance[count : count + batch_size, 0 : kernel_size - 1] *= head_kernel
            store_importance[
                count : count + batch_size, (max_seq_len - kernel_size + 1) : max_seq_len
            ] *= tail_kernel
