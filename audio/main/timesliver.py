...
class timesliver_network(nn.Module):
    def __init__(self, num_classes, d_model, d_out, max_m, rank):
        ...
        self.cnn2 = nn.Sequential( nn.Conv1d(self.d_model,16,1, stride=1), 
                                   ...
                                   nn.Conv1d(32,int(self.d_out),1, stride=1),
                                #    nn.ReLU(),
                                #    nn.Conv1d(int(self.d_out),int(self.d_out),1, stride=1),
                                   ) ##16 size motifs
        self.ln2 = nn.LayerNorm(self.d_out)
        
        # self.cnn3 = nn.Sequential( nn.Conv1d(self.d_model,16,2, stride=1), 
        #                            nn.ReLU(),
        #                            nn.Conv1d(16,32,2, stride=1),
        #                            nn.ReLU(),
        #                            nn.Conv1d(32,int(self.d_out),2, stride=1),
        #                            nn.ReLU(),
        #                            nn.Conv1d(int(self.d_out),int(self.d_out),2, stride=1),
        #                            ) ##16 size motifs
        ...
        # self.start = nn.LayerNorm([self.d_in,self.max_len])
        ...
        # self.motif_size_1 = 8*1
        self.motif_size_2 = 1*1
        # self.motif_size_3 = 16*1
        
        self.reduction = nn.Sequential(nn.AvgPool2d((1, 3), stride=(1, 3)),
                                    #    nn.AvgPool2d((1, 2), stride=(1, 2))
                                    )
                      
        self.nn = nn.Sequential(
                                # nn.Linear(int(sax_alpha*self.d_out*self.max_m),num_classes),
                                nn.Linear(int(self.d_out*133*self.max_m),num_classes),
                                # nn.ReLU(),
                                # nn.Linear(32,8),
                                # nn.ReLU(),
    ...
    def forward(self, x, sax, seq_len):
        ...
        # x = self.start(x)
        # x = self.start(x.permute(0,2,1)).permute(0,2,1)
        out = self.proj(x)
        # out = x
        ...
        # heat_map = heat_map.permute(0,1,3,2)
    ...
    def calculate_motif_level(self,dp,m_i, initial_cam):
        ...
        min_val_prot = torch.zeros((total,1)).to(self.rank)
        ...
        for i in range(q_id): # i is related to size of latent rep
            for j in range(d_id): # j in related to num of categorical var
                # for prot in range(total): # prot defines the protein number in a batch
                    ...
                    # min_val_prot[:,0] = torch.min(temp, dim=1)[0].squeeze()
                    
                    temp[:,:] = (temp)/(max_val_prot+1E-18)     
                    # temp[:,:] = (temp-min_val_prot)/(max_val_prot+1E-18-min_val_prot)              
