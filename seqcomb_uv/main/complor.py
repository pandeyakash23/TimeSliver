        ...
        for i in range(q_id): # i is related to size of latent rep
            for j in range(d_id): # j in related to num of categorical var
                # for prot in range(total): # prot defines the protein number in a batch
                    ...
                    # temp_pos = nn.ReLU()(signs*d_comp[:,j,:]*q_comp[:,i,:])
                    # temp_neg = nn.ReLU()(-signs*d_comp[:,j,:]*q_comp[:,i,:])
                    #normalization
                    # temp_pos_sum = torch.sum(temp_pos, dim=1).unsqueeze(-1)
                    # temp_neg_sum = torch.sum(temp_neg, dim=1).unsqueeze(-1)
                    # temp_pos = temp_pos/(temp_pos_sum+1E-18)
                    # temp_neg = temp_neg/(temp_neg_sum+1E-18)
