"""
Utility functions for TimeSliver - Synthetic Data.
...
def load_data(split, load_sax=True):
    ...
    """
    print(f"  Loading x_{split}.npy...", flush=True)
    x = np.load(config.get_data_path(split, "x"), allow_pickle=True)
    print(f"    x shape: {x.shape}", flush=True)

    print(f"  Loading y_{split}.npy...", flush=True)
    ...
    if load_sax:
        print(f"  Loading sax_{split}.npy...", flush=True)
        data["sax"] = np.load(config.get_data_path(split, "sax"), allow_pickle=True)
        print(f"    sax shape: {data['sax'].shape}", flush=True)
...
def evaluate_model(model, dataloader, device, num_samples):
    ...
    Returns:
        Dictionary with accuracy, confusion matrix, and other metrics
    """
    model.eval()
    predicted_labels = torch.zeros((num_samples, 1))
    predicted_probs = torch.zeros((num_samples, 1))
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            predicted_probs[count : count + batch_size, 0] = probs[:, 1]  # Prob of class 1
            actual_labels[count : count + batch_size, 0] = actual
            count += batch_size

    predicted_labels = predicted_labels.cpu().numpy().reshape(-1)
    predicted_probs = predicted_probs.cpu().numpy().reshape(-1)
    ...
    try:
        results["auc_roc"] = roc_auc_score(actual_labels, predicted_probs)
        results["precision"] = precision_score(actual_labels, predicted_labels)
        results["recall"] = recall_score(actual_labels, predicted_labels)
    except:
        pass
