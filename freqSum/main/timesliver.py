"""
TimeSliver Network Architecture for Synthetic Data Classification.
...
class TimeSliverNetwork(nn.Module):
    """
    TimeSliver Network for Synthetic Data Classification.

    Combines CNN-based motif extraction with SAX pooling for
    interpretable time series classification. Binary classification
    with direct input (no projection layer).
    """
    ...
    def __init__(self, num_classes, d_model, d_out, max_m, device):
        ...
        self.d_out = d_out
        self.sax_alpha = 90  # SAX alphabet size for synthetic data

        # Projection layer (optional, not used in forward by default)
        self.proj = nn.Linear(self.d_in, self.d_model)

        # Positional encoding (optional, not used by default)
        self.positional_encoding = PositionalEncoding(self.d_model)

        # CNN layers for motif extraction - Scale 1 (7-size motifs)
        ...
        # SAX pooling layers - Scale 2 (for potential multi-scale)
        ...
        self.nn = nn.Sequential(nn.Linear(int(9 * 11 * self.max_m), num_classes))
    ...
    def make_p(self, out, sax, seq_len, m):
        ...
        Args:
            out: Input [batch, features, seq_len]
    ...
    def forward(self, x, sax, seq_len):
        ...
        """
        # Direct input (no projection or positional encoding)
        out = x
        out = torch.permute(out, (0, 2, 1))  # [batch, features, seq_len]
    ...
    def forward_motif_importance(self, x, sax, seq_len):
        ...
        Returns:
            Tuple of (logits, cam gradients, initial_cam gradients, p_1)
        """
        ...
        return heat_map, self._cam, self._initial_out, p_1
