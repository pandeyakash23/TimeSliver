"""
Calculate temporal attribution scores for Synthetic Data samples.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate temporal attribution scores for Synthetic Data",
...
def calculate_attribution(model, dataloader, device, num_samples, max_seq_len):
    ...
    for batch_idx, (x, sax, classes, seq_len, labels) in enumerate(dataloader):
        ...
        for m_i in range(cam.size(-1)):
            mo_level_imp = model.calculate_motif_level_new(cam[..., m_i], m_i + 1, initial_cam)
